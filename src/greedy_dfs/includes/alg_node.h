...
struct AlgNode {
  AlgNode(const std::shared_ptr<AlgNode> &parent, const Board &board,
          double priority, const Move &move)
...
struct AlgNodeComparator {
  bool operator()(const std::shared_ptr<AlgNode> &lhs,
                  const std::shared_ptr<AlgNode> &rhs) const {
