...
int usage(const std::string &pname) {
  std::cout << "Usage: " << pname << " [board size] "
            << "[3 fp values in [0, 1] range]" << std::endl;
  return 1;
}
...
int main(const int argc, const char *argv[]) {
  ...
  auto weights = std::make_tuple(1.0, std::stod(argv[2]), std::stod(argv[3]),
                                 std::stod(argv[4]));
  ...
  Heuristics heuristics{number_of_free_positions_heuristic,
                        dist_to_center_heuristic, isolated_pegs_heuristic,
                        number_of_new_moves_heuristic};

  auto main_board = Board(boardSize);
  /* Initialisation of priority queue */
  std::priority_queue<std::shared_ptr<AlgNode>,
                      std::vector<std::shared_ptr<AlgNode>>, AlgNodeComparator>
      pq{};
  pq.emplace(std::make_shared<AlgNode>(nullptr, main_board, DBL_MAX,
                                       Move{-1, -1, jump_dir::EAST}));
  ...
  while (!pq.empty()) {
    ...
    for (auto move : moves) {
      ...
      if (!visited_boards.contains(tmp_board.current_state)) {
        ...
        auto normalised_values = std::apply(
            [&norm](auto &&...elems) {
              return std::make_tuple((static_cast<double>(elems) / norm)...);
            },
            heuristic_value);

        auto priority = get_euclidian_norm(multiply_two_tuples(
            normalised_values, weights,
