...
template <std::size_t... Is, typename... A, typename... B>
  requires((std::convertible_to<A, double> && ...) &&
           ...
           (sizeof...(A) == sizeof...(B)))
static auto multiply_two_tuples(const std::tuple<A...> &a,
                                const std::tuple<B...> &b,
...
static double get_sum_of_tuple_elements(const std::tuple<TupleArgs...> &tuple) {
  return std::apply(
      [](auto &&...args) { return (0.0 + ... + static_cast<double>(args)); },
      tuple);
}
...
static double get_euclidian_norm(const std::tuple<TupleArgs...> &tuple) {
  return std::apply(
      [](auto &&...args) {
        return std::sqrt(
            (0.0 + ... +
             (static_cast<double>(args) * static_cast<double>(args))));
      },
      tuple);
}

template <Heuristic... Hs> struct Heuristics {
  // ------------------------------
  // Class creation
  // ------------------------------
  explicit Heuristics(Hs... heuristics)
      : heuristic_functions(std::make_tuple(std::move(heuristics)...)) {};
  ~Heuristics() = default;
  ...
  auto calculate_heuristics(const Board &b) {
    return std::apply([&b](auto &&...hs) { return std::make_tuple(hs(b)...); },
                      heuristic_functions);
  }
  ...
  auto multiply_with_weights(const std::tuple<Weights...> &weights,
                             const std::tuple<Values...> &values) {
    return multiply_two_tuples(weights, values,
                               std::index_sequence_for<Weights...>{});
  }

  template <typename... Weights>
    requires(std::convertible_to<Weights, double> && ... &&
             (sizeof...(Weights) == sizeof...(Hs)))
  auto multiply_with_weights(const std::tuple<Weights...> &weights,
                             const Board &b) {
