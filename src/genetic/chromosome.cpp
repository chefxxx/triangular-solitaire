...
void Chromosome::normalizeGenes() {
  auto norm = std::accumulate(
      genes.begin(), genes.end(), 0.0f,
      [](const float acc, const float x) { return acc + x * x; });
  norm = std::sqrt(norm);
  for (auto &gene : genes) {
    gene /= norm;
  }
...
void printPopulation(const std::vector<Chromosome> &population,
  ...
  const int count =
      std::count_if(population.begin(), population.end(),
                    [](const Chromosome &c) { return c.board.pegs_left == 1; });
...
std::ostream &operator<<(std::ostream &os, const Chromosome &chromosome) {
  ...
  os << "} score: " << chromosome.score
     << ", left: " << chromosome.board.pegs_left << " ";
