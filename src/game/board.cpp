...
Board::Board(const int size)
    : board_size{size}, board_area_mask{generate_board()},
...
tl::expected<jump_dir, board_error_info>
Board::move_peg(const peg_position &from, const peg_position &to) {
  if (!CheckBitAtIdx(board_area_mask, peg_to_idx(from)))
    return tl::unexpected{
        board_error_info(board_error::out_of_bound, from, to)};
  if (!CheckBitAtIdx(board_area_mask, peg_to_idx(to)))
    return tl::unexpected{
        board_error_info(board_error::out_of_bound, from, to)};
  if (!CheckBitAtIdx(current_state, peg_to_idx(from)))
    return tl::unexpected{
        board_error_info(board_error::peg_does_not_exist, from, to)};
  if (CheckBitAtIdx(current_state, peg_to_idx(to)))
    return tl::unexpected{
        board_error_info(board_error::invalid_jump, from, to)};
...
tl::expected<jump_dir, board_error_info> Board::move_peg(const int from,
                                                         const int to) {
...
tl::expected<void, board_error_info> Board::undo_move(const peg_position &from,
                                                      const peg_position &to,
                                                      const jump_dir &dir) {
  // Same checks as in move_peg function, but in reversed order, now it's
  // necessary that from is free and to is occupied
  if (!CheckBitAtIdx(board_area_mask, peg_to_idx(from)))
    return tl::unexpected{
        board_error_info(board_error::out_of_bound, from, to)};
  if (!CheckBitAtIdx(board_area_mask, peg_to_idx(to)))
...
tl::expected<void, board_error_info> Board::undo_move(int from, int to,
                                                      const jump_dir &dir) {
  const peg_position p_from{from};
  const peg_position p_to{to};
...
void perft(Board &board, bool debug) {
  ...
  while (!moves_stack.empty()) {
    ...
    if (!state) {
      ...
      if (!exp.has_value())
        std::cout << exp.error().message() << " in undo move from " << move.from
                  << "to " << move.to << "\n";
      else
        std::cout << "Return to state before move from:" << move.from << " to "
                  << move.to << "\n";
    }
  }
}
