...
std::vector<Move> BuildAllMoves(const Board &board) {
  ...
  const uint64_t north =
      shiftNorth(shiftNorth(current) & current & area) & empty & area;
  const uint64_t south =
      shiftSouth(shiftSouth(current) & current & area) & empty & area;
  const uint64_t west =
      shiftWest(shiftWest(current & notAB_Files) & current & area) & empty &
      area;
  const uint64_t east =
      shiftEast(shiftEast(current & notGH_Files) & current & area) & empty &
      area;
  const uint64_t northWest =
      shiftNorthWest(shiftNorthWest(current & notAB_Files) & current & area) &
      empty & area;
  const uint64_t southEast =
      shiftSouthEast(shiftSouthEast(current & notGH_Files) & current & area) &
      empty & area;
...
void MakeMoves(uint64_t attacks, const jump_dir offset,
               std::vector<Move> &moves) {
  while (attacks) {
