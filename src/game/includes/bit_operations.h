...
constexpr bool CheckBitAtIdx(const uint64_t a, const int idx) {
  return a & (MinMsb << idx);
}

/* function clears bits intersecting with b from a */
[[nodiscard]] constexpr uint64_t ClearIntersect(const uint64_t a,
                                                const uint64_t b) {
  return a ^ (a & b);
}
...
constexpr int PopLsb(uint64_t &a) {
  const int ret = GetLbsIdx(a);
  a &= a - 1;
  return ret;
}
